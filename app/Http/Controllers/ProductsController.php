...
class ProductsController extends Controller
{
	public function addProduct(Request $request){
		...
		if($request->isMethod('post')){
            ...
            if($request->hasFile('pdf')){
             $pdf_tmp = Input::file('pdf');
             $pdf_name = $pdf_tmp->getClientOriginalName();
             $pdf_path = '/pdfs';
             $pdf_tmp->move($pdf_path,$pdf_name);
             $product->pdf = $pdf_name;
            }
	...
	public function editProduct(Request $request,$id=null){
		...
		if($request->isMethod('post')){
            ...
            if($request->hasFile('pdf')){
             ...
             $pdf_tmp->move($pdf_path,$pdf_name);
             $pdfName = $pdf_name;
            }else if(!empty($data['current_pdf'])){
                $pdfName = $data['current_pdf'];
            }else{
                $pdfName = '';
            }
			...
			Product::where(['id'=>$id])->update(['feature_item'=>$feature_item,'status'=>$status,'category_id'=>$data['category_id'],'product_name'=>$data['product_name'],
				'product_code'=>$data['product_code'],'description'=>$data['description'],'price'=>$data['price'],'stock'=>$data['stock'],'image'=>$fileName,'pdf'=>$pdfName]);
    ...
    public function deleteProductPdf($id){
        $productPdf = Product::select('pdf')->where('id',$id)->first();

        $video_path = 'pdfs/';

        if(file_exists($pdf_path.$productPdf->pdf)){
            unlink($pdf_path.$productPdf->pdf);
        }

        Product::where('id',$id)->update(['pdf'=>'']);
        return redirect()->back()->with('flash_message_success','Product PDF has been deleted successfully!');
    }
    ...
    public function products($url=null){
    	...
    	return view('products.listing')->with(compact('categories','productsAll','categoryDetails','meta_title','meta_description','meta_keywords'));
    }
    ...
    public function addtocart(Request $request){
        ...
        //Check product stock is it available or not?
        ...
        if($countProducts>0){
           ...
           $countProducts = DB::table('cart')->where(['product_id' => $data['product_id'],
            'session_id' => $session_id , 'user_email' => $data['user_email']])->count();
        if($countProducts>0){
            return redirect()->back()->with('flash_message_error','Product already exist in Cart!');
           }
        }
    ...
    public function cart(){           
        ...
        $meta_keywords = "shopping cart,ecom website";
